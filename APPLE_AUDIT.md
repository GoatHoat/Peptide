...
| **1.4.2** | **Yes** — a calculator exists | **pass, with the caveat below** | **Correction.** An earlier version of this row said `legal.md` records a rejection on this guideline. **It does not.** `legal.md:5` describes a *design* that was abandoned — dosing and administration protocols tied to peptide entries — and says it "was tested across several variations", with no statement about who rejected it or whether it ever reached review. I read "tested" as "submitted" and wrote a rejection that is not in the file. There is no record of any rejection anywhere in the repo. Re-judged on the code alone: `src/screens/ReconCalculator.tsx` takes three numbers the user types and converts between units. It is reachable only from Today, is attached to no product, has no default, suggests no amount and persists nothing (`src/lib/recon.ts`). No peptide entry carries an amount — `supabase/migrations/0031_peptides_have_no_dose.sql`, and `tests/e2e/peptides.spec.ts` asserts it on the row, the opened card and the Add-to-Schedule search. The assistant refuses peptide questions server-side before the model sees them (`supabase/functions/ask/lib.ts:294-303`, `classifyScope`). 1.4.2's actual text is about *drug dosage calculators*; a unit converter that is given every input and names no drug is not one. **The caveat that remains is real but is mine, not Apple's**: this is a judgement about where a line falls, and if review disagrees the screen removes cleanly — delete the `recon` case in `Today.tsx` and the two files. |
